...
public class RestControllerTest {
    ...
    // 테스트 주소
    // 웹 브라우저 주소창에서 시작
    // (우리 도메인에서 요청 주소는 http://localhost:8080/todos/1 보낼 예정)
    // https://jsonplaceholder.typicode.com/todos/1

    @GetMapping("/todos/{id}")
    public ResponseEntity<?> test(@PathVariable(name = "id") Integer id) {
        ...
        // 사용자가 요청한 우리 웹 브라우저 출력됨
        return ResponseEntity.status(HttpStatus.OK).body(requestHttpMessage.getBody());
...
//    },
        ...
        PostDTO postDTO = PostDTO
                ...
                .body("헤더를 마음대로 설정 가능, GET , POST, PUT, DELETE 다 사용가능")
                .build();
        ...
        System.out.println("저장된 바디 :" + resultBody.getBody());
